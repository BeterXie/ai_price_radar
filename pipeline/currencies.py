...
GEL GHS GIP GMD GNF GTQ GYD HKD HNL HTG HUF IDR ILS INR IQD IRR ISK JMD JOD JPY KES KGS KHR KMF KPW
...
VED VES VND VUV WST XAF XAG XAU XBA XBB XBC XBD XCD XDR XOF XPD XPF XPT XSU XTS XUA XXX YER ZAR ZMW ZWG
""".split())


def normalize_currency(value: object, default: str = "CNY") -> str:
    currency = str(value or default).strip().upper()
    if currency not in ISO_4217_CODES:
        raise ValueError(f"unsupported currency: {currency or '<empty>'}")
    return currency
