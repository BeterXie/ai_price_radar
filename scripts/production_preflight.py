...
def validate_production_env(env: Dict[str, str]) -> List[str]:
    ...
    if (
        ...
        or session_secret_key in PLACEHOLDERS
        or len(session_secret_key.encode()) < 32
    ):
        errors.append("SESSION_SECRET_KEY must contain at least 32 random bytes")
    if env.get("QQ_MOCK_AUTH_ENABLED", "").casefold() in {"true", "1", "yes", "on"}:
        errors.append("QQ_MOCK_AUTH_ENABLED must be false in production")
    if env.get("DEV_PRINT_AUTH_CODES", "").casefold() in {"true", "1", "yes", "on"}:
        errors.append("DEV_PRINT_AUTH_CODES must be false in production")
    if env.get("EXPOSE_API_DOCS", "").casefold() not in {"false", "0", "no"}:
        errors.append("EXPOSE_API_DOCS must be false in production")
