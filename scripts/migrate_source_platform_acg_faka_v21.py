...
ALTER TABLE source_intakes ADD CONSTRAINT ck_source_intakes_type CHECK (
    source_type IN (
        ...
        'woocommerce', '16688', 'schema_org', 'acg_faka', 'other'
    )
);
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Allow acg_faka source intakes and discovery candidates")
    ...
    with psycopg.connect(connection_url(args.database_url)) as connection:
        migrate(connection)
        connection.commit()
    print("acg_faka source platform v21 migration complete")
    return 0
