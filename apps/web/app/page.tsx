...
export default async function HomePage() {
  const data = await getProducts("in_stock=true&sort=price");
  const products = data.items.slice(0, 6);
  const totalOffers = products.reduce((sum, item) => sum + item.offer_count, 0);
  const inStock = products.reduce((sum, item) => sum + item.in_stock_count, 0);
  return (
    <main>
      <section className="grid-noise border-b hairline">
        <div className="shell grid min-h-[620px] items-stretch lg:grid-cols-[1.45fr_.55fr]">
          <div className="flex flex-col justify-between border-r-0 hairline py-14 lg:border-r lg:pr-14 lg:py-20">
            <div>
              <p className="mono flex items-center gap-3 text-xs uppercase tracking-[.18em]"><span className="signal-dot" /> public price intelligence</p>
          ...
          <aside className="flex flex-col justify-end py-14 lg:pl-10 lg:py-20">
            <div className="border-t border-black pt-5">
              <p className="mono text-xs uppercase tracking-[.14em] text-black/45">Live catalog</p>
              <div className="mt-6 space-y-8">
                <div><p className="text-5xl font-semibold tracking-[-.06em]">{products.length}</p><p className="mt-1 text-sm text-black/50">标准产品类别</p></div>
                <div><p className="text-5xl font-semibold tracking-[-.06em]">{totalOffers}</p><p className="mt-1 text-sm text-black/50">当前有效报价</p></div>
                <div><p className="text-5xl font-semibold tracking-[-.06em]">{inStock}</p><p className="mt-1 text-sm text-black/50">有货报价</p></div>
              </div>
            </div>
          </aside>
        </div>
      ...
      <section className="shell py-20">
        <div className="flex items-end justify-between gap-6 border-b border-black pb-5">
          <div><p className="mono text-xs uppercase tracking-[.14em] text-black/45">Market board</p><h2 className="mt-3 text-4xl font-semibold tracking-[-.05em]">当前有货低价</h2></div>
      ...
      <section className="border-y border-black bg-[color:var(--accent)] text-[color:var(--accent-ink)]">
        <div className="shell grid lg:grid-cols-[.7fr_1.3fr]">
          <div className="border-b border-black py-12 lg:border-b-0 lg:border-r lg:pr-12"><p className="mono text-xs uppercase tracking-[.15em]">Trust model</p><h2 className="mt-5 text-5xl font-semibold leading-none tracking-[-.06em]">不替渠道背书。<br />只保留可回看的事实。</h2></div>
