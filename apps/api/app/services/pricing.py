...
def price_median(values: Iterable[Decimal | None]) -> Decimal | None:
    ...
    normalized = [value for value in values if value is not None and value.is_finite() and value > 0]
    if not normalized:
        return None
...
def low_price_warning(price: Decimal | None, median_price: Decimal | None, currency: str = "CNY") -> str | None:
    ...
    if median_price is not None and median_price.is_finite() and median_price > 0 and price < median_price * LOW_PRICE_RATIO:
...
def is_trusted_price(price: Decimal | None, median_price: Decimal | None) -> bool:
    ...
    return price is not None and price.is_finite() and price > 0 and low_price_warning(price, median_price) is None
