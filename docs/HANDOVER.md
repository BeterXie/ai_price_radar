...
生产流程已自动化：扫描成功后先校验 SQLite，再生成并仅保留最新一份快照，最后幂等同步 PostgreSQL；不需要人工执行导入。若一轮没有任何成功店铺，任务会在生产导入前退出，保留线上最后一次成功数据。
...
公开 API：产品列表、产品详情、店铺详情、筛选元数据、举报提交和店铺收录申请。产品和店铺详情报价会返回原始标题、原始分类、纯文本描述、商品类型、市场标价、首次/最后发现时间、价格、库存、交付方式和原站链接；原始 HTML、内部用户字段和联系方式结构不会直接输出。店铺收录申请会校验链动小铺公开链接、按店铺 token 去重，并进入现有后台审核队列；申请联系方式不会公开。管理 API：统计、报价列表、报价修改、重新分类、举报列表和举报状态修改。管理员接口使用 `X-Admin-Key`。

报价目录采用带真实品牌图标的平台标签和标准商品标签两级快捷筛选。全部商品、单一平台和单一标准商品使用相同的报价概况、“仅显示可直接比较”、“仅看有货”、“有质保”与“无质保”即时筛选、同款报价展开和瀑布流加载；跨商品范围会额外标注所属标准商品。标准产品使用 `/products/{slug}` 清晰 URL，但仍直接渲染同一目录工作区，不恢复旧详情页 UI；旧的 `?platform=...&product=...` 地址会永久重定向到清晰 URL。标准产品主最低价只统计可直接比较报价，相关商品最低价单独展示。同款报价首屏仅返回 30 个分组，滚动接近列表底部时继续分页；展开同款后再加载全部店铺报价和原始描述。每个公开页面标注已发布快照编号和北京时间。站点提供 canonical、robots.txt、按有效报价和真实更新时间生成的动态 sitemap.xml、不含虚构评分的 Product / AggregateOffer 结构化数据，以及产品独特说明、FAQ 与分享图。没有有效报价的产品页保持可抓取但标记 `noindex,follow`，并从 Sitemap 移除。公开页不展示管理入口，管理页面仍可通过已知的 `/admin` 路径访问。公开页头和页脚提供 `/shops/submit` 店铺收录入口，申请审核通过并完成抓取验证后才会进入报价目录。
...
P2：可编辑 SEO 内容后台、更多来源适配器、价格异常检测、店铺申诉工作流、搜索服务和数据分析。
