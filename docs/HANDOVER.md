...
报价目录采用带真实品牌图标的平台标签和标准商品标签两级快捷筛选。全部商品、单一平台和单一标准商品使用相同的报价概况、“仅显示可直接比较”与“仅看有货”即时筛选、同款报价展开和瀑布流加载；跨商品范围会额外标注所属标准商品。选择标准商品标签后，产品工作区直接在目录导航下方展开，不再进入独立详情页；历史详情地址会永久重定向到对应的目录内联视图。标准产品主最低价只统计可直接比较报价，相关商品最低价单独展示。同款报价首屏仅返回 30 个分组，滚动接近列表底部时继续分页；展开同款后再加载全部店铺报价和原始描述。每个公开页面标注已发布快照编号和北京时间。站点提供 canonical、robots.txt、sitemap.xml 和不含虚构评分的 Product / AggregateOffer 结构化数据。公开页不展示管理入口，管理页面仍可通过已知的 `/admin` 路径访问。公开页头和页脚提供 `/shops/submit` 店铺收录入口，申请审核通过并完成抓取验证后才会进入报价目录。
