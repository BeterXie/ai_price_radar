...
发布保障：失败的爬虫扫描不会删除网站最后一次成功数据；同一店铺和商品重复导入不会产生重复报价；价格、库存或状态发生变化时才新增历史记录；超过 72 小时的报价不再参与最低价计算。
...
报价目录采用带真实品牌图标的平台标签和标准商品标签两级快捷筛选，点击后直接更新结果并保留可分享的查询参数；排序、库存和细粒度标签保留在辅助筛选区。目录使用紧凑统计和全宽产品列表；商品详情首屏仅返回 30 条报价，滚动接近列表底部时再通过分页 API 每次加载 30 条，避免把全部报价塞入首屏。每条报价可继续展开查看清洗后的商家原始描述和完整时间信息。站点图标由 `apps/web/app/icon.svg` 提供，同时用于浏览器标签页和页头品牌标记。公开页不展示管理入口，管理页面仍可通过已知的 `/admin` 路径访问。
